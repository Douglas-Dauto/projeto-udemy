...
let controlMode = JSON.parse(localStorage.getItem("controlSwitch"));
let containerSwitchMode;
...
function switchMode(mode) {
    ...
    containerSwitchMode.addEventListener("click", () => {
        switchMode(controlMode);
...
async function insertCourses() {
    if (controlMode) {
        ...
        .then(data => {
            console.log(data)
            for (let i = 0; i <= 29; i++) {
                courses.push(new Course(data[i].image, data[i].title, data[i].createdBy, data[i].starNote, data[i].popularity, data[i].valueMoney, data[i].classification));
            }
        })
        .catch(error => {
